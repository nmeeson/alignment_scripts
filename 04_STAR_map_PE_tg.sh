...
#SBATCH --output=bin/OUT/star_tg/%A_%a-%J.o
#SBATCH --error=bin/ERR/star_tg/%A_%a-%J.e
...
mkdir -p ${myDir}/output/${sampleNames[$SLURM_ARRAY_TASK_ID]}/star_tg
#mkdir -p ${myDir}/bin/OUT/star_tg/
#mkdir -p ${myDir}/bin/ERR/star_tg/

module load STAR

STAR --readFilesCommand zcat --outSAMunmapped Within KeepPairs --outMultimapperOrder Random --outSAMmultNmax 1 --runThreadN 1 --runMode alignReads --quantMode GeneCounts --outSAMtype BAM SortedByCoordinate --outFileNamePrefix ${myDir}/output/${sampleNames[$SLURM_ARRAY_TASK_ID]}/star_tg/${sampleNames[$SLURM_ARRAY_TASK_ID]}.onemap. --genomeDir ${myDir}/resources/ --readFilesIn ${myDir}/output/${sampleNames[$SLURM_ARRAY_TASK_ID]}/trimgalore/${sampleNames[$SLURM_ARRAY_TASK_ID]}_1_val_1.fq.gz ${myDir}/output/${sampleNames[$SLURM_ARRAY_TASK_ID]}/trimgalore/${sampleNames[$SLURM_ARRAY_TASK_ID]}_2_val_2.fq.gz
